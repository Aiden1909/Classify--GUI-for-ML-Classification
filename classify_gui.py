...
class MachineLearning:
    ...
    def select(self):
        try:
            ...
            self.hist_plot['state'] = NORMAL
            self.heatmap['state'] = NORMAL
            #self.corr_plot['state'] = NORMAL
            #self.pair_plot['state'] = NORMAL
    ...
    def hide(self):
        ...
        self.df_show['state'] = DISABLED
        self.df_hide['state'] = DISABLED
        self.name_select['state'] = NORMAL
        self.select_x['state'] = DISABLED
        self.select_y['state'] = DISABLED
        self.hist_plot['state'] = DISABLED
        self.heatmap['state'] = DISABLED
        self.corr_plot['state'] = DISABLED
        self.pair_plot['state'] = DISABLED
        self.feature_change['state'] = DISABLED
        self.split_button['state'] = DISABLED

        self.svm_shap['state'] = DISABLED
        self.log_shap['state'] = DISABLED
        self.dt_shap['state'] = DISABLED
        self.rf_shap['state'] = DISABLED    

        self.name.set('')

    def create_hplot(self):
        self.hplot= Hplot(self.data,self.window)

    def create_hmap(self):
        self.hmap= Hmap(self.data,self.window)       

    def create_cplot(self):
    ...
    def get_y(self):
        ...
        if self.y not in self.X:
            ...
            self.corr_plot['state'] = NORMAL
            self.pair_plot['state'] = NORMAL
            self.feature_change['state'] = NORMAL
    ...
    def pred_svm(self):
        self.model= SVC_predict_choice(self.window)
        self.C = int(self.model.choice)
        self.svm_model = SVC(C=self.C,kernel=self.model.n1.get(),probability=True)
        self.svm_model.fit(self.X_train, self.y_train)
        self.svm_predictions = self.svm_model.predict(self.X_test)

        self.svm_error['state'] = NORMAL
        self.svm_cr['state'] = NORMAL
        self.svm_cm['state'] = NORMAL
        self.svm_shap['state'] = NORMAL

    def cm_svm(self):
        ConfusionMatrix(self.window, confusion_matrix(self.le.inverse_transform(self.y_test), self.le.inverse_transform(self.svm_predictions)), 'SVM', self.le.classes_)

    def cr_svm(self):
        ClassificationReport(self.window, classification_report(self.le.inverse_transform(self.y_test), self.le.inverse_transform(self.svm_predictions)), 'SVM')    

    def errors_svm(self):
        temp = [mean_absolute_error(self.y_test, self.svm_predictions), mean_squared_error(self.y_test, self.svm_predictions), np.sqrt(mean_squared_error(self.y_test, self.svm_predictions))]
        Errors(self.window, temp, 'SVM')

    def svm_create_splot(self):
        self.le.fit(self.data[self.y])
        SummaryPlot(self.window,self.svm_model,self.X_train,self.X_test,'SVM',self.le.classes_)    

    def pred_logreg(self):
        self.model= log_predict_choice(self.window)
        self.C = int(self.model.choice)
        self.logreg_model = LogisticRegression(C=self.C,solver=self.model.n1.get(),max_iter=10000)
    ...
    def lr_create_splot(self):
        self.le.fit(self.data[self.y])
        SummaryPlot(self.window,self.logreg_model,self.X_train,self.X_test,'SVM',self.le.classes_)    

    def pred_dtree(self):
        self.model= dt_predict_choice(self.window)
        self.depth = int(self.model.choice)
        if self.depth==0:
            self.depth=None
        self.dtree_model = DecisionTreeClassifier(criterion=self.model.n1.get(),max_depth=self.depth)
    ...
    def dt_create_splot(self):
        self.le.fit(self.data[self.y])
        SummaryPlot(self.window,self.dtree_model,self.X_train,self.X_test,'Decision Tree',self.le.classes_)    

    def pred_rforest(self):
        self.model= dt_predict_choice(self.window)
        self.rdepth = int(self.model.choice)
        if self.rdepth==0:
            self.rdepth=None
        self.rforest_model = RandomForestClassifier(n_estimators=100,criterion=self.model.n1.get(),max_depth=self.rdepth)
...
class SVC_predict_choice():
    def __init__(self,master):
        ...
        self.window.title(" SVM Prediction Parameters")
        ...
        self.kernel_label= Label(self.canvas,text='Type of Kernel')
        self.kernel_label.place(x=20,y=20)

        self.C_label= Label(self.canvas,text='Regularisation Parameter')
        self.C_label.place(x=20,y=80)
        self.choice=None 

        self.n1 = StringVar()
        self.kernelchosen = ttk.Combobox(self.canvas, width = 27, textvariable = self.n1)
        self.kernelchosen['values']= ('linear', 'poly', 'rbf', 'sigmoid', 'precomputed')
        self.kernelchosen.place(x=20,y=40)

        self.Cbox=IntVar()
        self.Cbox= ttk.Spinbox(self.canvas, from_=1, to = 10, width=5)
        self.Cbox.place(x=20,y=100)
        ...
        self.kernelchosen.current(1)

        self.window.mainloop()
    
    def confirm(self):

        self.choice=self.Cbox.get()
        self.window.grab_release()
        self.window.quit()
        self.window.destroy()

class log_predict_choice():
    def __init__(self,master):
        self.master=master

        self.window=Toplevel(self.master)
        self.window.grab_set()
        self.window.title(" Logistic Regression Prediction Parameters")
        self.window.geometry('400x400')
        self.window.minsize(250, 250)
        ...
        self.solver_label= Label(self.canvas,text='Type of Solver')
        self.solver_label.place(x=20,y=20)
        ...
        self.n1 = StringVar()
        self.solverchosen = ttk.Combobox(self.canvas, width = 27, textvariable = self.n1)
        self.solverchosen['values']= ('newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga')
        self.solverchosen.place(x=20,y=40)
...
class dt_predict_choice():
    def __init__(self,master):
        self.master=master

        self.window=Toplevel(self.master)
        self.window.grab_set()
        self.window.title(" Decision Tree Prediction Parameters")
        self.window.geometry('400x400')
        self.window.minsize(250, 250)
        ...
        self.criterion_label= Label(self.canvas,text='Type of Criterion')
        self.criterion_label.place(x=20,y=20)

        self.maxdepth_label= Label(self.canvas,text='Max Depth Of Tree(0 for None)')
        self.maxdepth_label.place(x=20,y=80)
        self.choice=None 

        self.n1 = StringVar()
        self.criterionchosen = ttk.Combobox(self.canvas, width = 27, textvariable = self.n1)
        self.criterionchosen['values']= ('gini', 'entropy')
        self.criterionchosen.place(x=20,y=40)

        self.Cbox=IntVar()
        self.Cbox= ttk.Spinbox(self.canvas, from_=0, to = 10, width=5)
        self.Cbox.place(x=20,y=100)
        ...
        self.canvas.configure(scrollregion=self.canvas.bbox(ALL))
        self.criterionchosen.current(1)
...
class rf_predict_choice():
    def __init__(self,master):
        self.master=master

        self.window=Toplevel(self.master)
        self.window.grab_set()
        self.window.title(" Random Forest Prediction Parameters")
        self.window.geometry('400x400')
        self.window.minsize(250, 250)
...
class Hplot:
    def __init__(self,data,master):
        ...
        self.figure = Figure(figsize=(10, 10), dpi=70)
        self.sub = self.figure.add_subplot(971,projection='polar')
        data.hist(bins=50,ax=self.sub)
...
class Hmap:
    def __init__(self,data,master):
        ...
        self.window = Toplevel(self.master)
        self.window.title("Heatmap")
        self.window.configure(background='white')
        #self.window.resizable(False, False)

        def create_plot(data):
            init_figure = Figure(figsize=(6, 6),dpi=70)
            ax = init_figure.subplots()
            #ax.set_yticklabels(data.columns,rotation=0)
            g=sns.heatmap(data.corr(),annot=True,cmap='coolwarm',ax=ax)
            g.set_xticklabels(g.get_xticklabels(), rotation=90)
            g.set_yticklabels(g.get_yticklabels(), rotation=0)
            return init_figure


        self.figure = create_plot(self.data)
        self.figure.tight_layout()
...
class Cplot:
    def __init__(self,data,master,target,features):
        ...
        self.window = Toplevel(self.master)
        self.window.title("Correlation Plot")
        self.window.configure(background='white')
        #self.window.resizable(False, False)

        def create_plot(data):
            init_figure = Figure(figsize=(6, 6),dpi=80)
            ax = init_figure.subplots()
            #ax.set_yticklabels(data.columns,rotation=0)
            p1=data[self.features]
            p2=data[self.target]
            p_merge=p1.merge(p2, how='inner',left_index=True,right_index=True)
            corr_mat=p_merge.corr()
            g=corr_mat[target].sort_values(ascending=False).plot(kind= 'bar',ax=ax)
            g.set_xticklabels(g.get_xticklabels(), rotation=90)
            return init_figure
...
class Pplot:
    def __init__(self,data,master,target):
        ...
        self.vis.image = img
        self.vis.pack(expand=True,fill=BOTH)
...
class SummaryPlot:
    def __init__(self,master,model,train,test,name,labels):
        ...
        self.g=shap.summary_plot(self.shap_values, self.test,class_names=self.labels)

        '''self.window = Toplevel(self.master)
        self.window.title(self.name + " Outcome Summary Plot")
        self.window.resizable(False, False)'''


        '''self.g.savefig('plot.png')
        img = ImageTk.PhotoImage(Image.open('plot.png').resize((1280,720)))
        self.vis = Label(self.window,image=img)
